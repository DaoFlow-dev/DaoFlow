...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION USING
      ...
      HINT = 'Each deployment must match a service with the same project, environment, service name, and source type before rerunning migration 0040.';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION USING
      ...
      HINT = 'Resolve duplicate service records so every deployment matches exactly one service before rerunning migration 0040.';
  END IF;
