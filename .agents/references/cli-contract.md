...
`backup recovery` operates on encrypted DaoFlow control-plane bundles in an existing backup
destination. The server enforces owner-only access. The CLI and UI expose only the recovery-key
fingerprint and rotation timestamp; they never print key material, credentials, secret values, or
private keys.

- `backup recovery plan --destination <id>` calls `controlPlaneRecoveryPlan` and is read-only.
- `backup recovery run --destination <id> --dry-run` calls the plan procedure and exits `3`.
- `backup recovery run --destination <id> --yes` calls `triggerControlPlaneRecoveryBundle` and
  requires `backup:run`.
- `backup recovery restore` is a local offline command. It does not require a running DaoFlow API.
  `--dry-run` validates the signed sidecar, encrypted bundle, protected external secrets, installed
  version, Compose database override, and new database name, then exits `3` with a deterministic
  plan hash. Execution requires both `--confirm <exact-plan-hash>` and `--yes`.
- Restore execution stops the control plane, creates and restores only a new database, runs
  migrations before switchover, atomically updates `DAOFLOW_DATABASE_NAME`, force-recreates DaoFlow,
  and verifies login plus projects, servers, audit history, and backups. The old database and a
  mode-`0600` copy of the old config remain available. Post-start failure restores the old config.
- Restore JSON output must never include secret values, passwords, database URLs, or session tokens.
- `backup recovery list` calls `controlPlaneRecoveryBundles`.
- `backup recovery inspect --bundle <id>` calls `controlPlaneRecoveryBundle`.
- `backup recovery download-metadata --bundle <id>` calls
  `controlPlaneRecoveryBundleMetadata` and shows safe manifest metadata only.

Every recovery command supports `--json` and uses the standard `{ "ok": true, "data": ... }`
envelope. A successful bundle requires checksum and isolated verification evidence; an upload or
verification failure must remain failed and include operator next steps.
...
- Scope: `env:read`
- Scope selection:
  - `daoflow env list --project-id <id>` lists only shared defaults for that project
  - `daoflow env set --project-id <id>` and `daoflow env delete --project-id <id>` create, update, and delete shared project defaults
  - environment and project selectors are mutually exclusive; project defaults cannot be branch-scoped
- Precedence is fixed: repository defaults < project < environment < service < preview environment < preview service. Deleting a more-specific value reveals the inherited value without copying it.
- Secret visibility:
...
- Audit contract:
  ...
  - writes require `env:write` and the selected project or environment must belong to the caller's team
- Inventory and deployment snapshot contract:
  - variable inventory returns the winning `origin` and monotonic database `revision`; secret values remain masked without `secrets:read`
  - each new immutable deployment snapshot records an origin and revision for every resolved entry; repository defaults use a deterministic content-derived `sha256:` revision and remain redacted in `configSnapshot.composeEnv`
...
- Direct compose execution must leave the project selector unset. The control plane derives the stack name from the Compose document and creates or reuses that project; the CLI must not reinterpret the filename or top-level Compose name as a required pre-existing project.
...
- Optional input:
  ...
  - `--json`
- JSON list success shape:
  - `{ "ok": true, "data": { "limit": number, "summary": { "totalRequests": number, "pendingRequests": number, "approvedRequests": number, "rejectedRequests": number, "criticalRequests": number }, "requests": [{ "id": string, "actionType": "compose-release" | "backup-restore" | "preview-deployment", "targetResource": string, "reason": string, "status": string, "requestedByUserId": string, "requestedByEmail": string | null, "requestedByRole": string | null, "resolvedByUserId": string | null, "resolvedByEmail": string | null, "inputSummary": object | null, "createdAt": string, "resolvedAt": string | null, "requestedBy": string, "resourceLabel": string, "riskLevel": "medium" | "elevated" | "critical", "statusTone": "healthy" | "failed" | "running" | "queued", "commandSummary": string, "requestedAt": string, "expiresAt": string, "decidedBy": string | null, "decidedAt": string | null, "recommendedChecks": string[], "dispatchStatus": "pending" | "retrying" | "dispatched" | "succeeded" | "terminal-failure" | null, "dispatchStatusLabel": string | null, "dispatchStatusTone": "healthy" | "failed" | "running" | "queued" | null, "operationId": string | null, "dispatchAttempts": number, "dispatchError": string | null, "dispatchNextAttemptAt": string | null, "dispatchedAt": string | null, "dispatchCompletedAt": string | null }] } }`
- Human output must show:
  ...
  - durable dispatch status, operation ID, and attempt count when a decision has created an execution intent
  - terminal dispatch error details when execution can no longer proceed
...
- Template apply leaves the direct-deploy project selector unset and carries the requested project name in the rendered Compose document, so first use creates the project and later use reuses it.
