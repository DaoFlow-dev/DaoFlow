  ...
  return new Command("status")
    ...
    .action(async (opts) => {
      ...
      try {
        ...
        console.log(`  Total: ${servers.summary.totalServers}  Ready: ${chalk.green(servers.summary.readyServers)}  Attention: ${chalk.yellow(servers.summary.attentionServers)}`);
        console.log();
      ...
      try {
        const deployments = await api.get<Array<{
          id: string;
          serviceName: string;
          status: string;
          createdAt: string;
        ...
        if (Array.isArray(deployments)) {
          for (const dep of deployments) {
            const statusColor =
              dep.status === "completed" ? chalk.green :
