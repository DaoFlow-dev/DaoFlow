    ...
    <div className="space-y-3">
      ...
      {entries.length === 0 ? (
        ...
        <div
          role="log"
          aria-live="polite"
          className="bg-zinc-950 text-zinc-100 border border-zinc-800 rounded-md p-3 max-h-72 overflow-y-auto font-mono text-xs leading-relaxed"
        >
          {entries.map((entry, i) => {
            ...
            return (
              <div key={i} className="flex gap-2 hover:bg-muted/50 px-1 py-0.5">
                {timestamp && <span className="text-zinc-400 shrink-0">{timestamp}</span>}
                <Badge
                  variant={
