...
export const roleCapabilities: Record<AppRole, readonly ApiTokenScope[]> = {
  ...
  // The agent role defines the MAXIMUM capabilities any agent token can have.
  // Individual presets (read-only, minimal-write, full) limit further.
  // Excludes: terminal:open, policy:override, members:manage, tokens:manage
  agent: [
    "server:read",
    "server:write",
    "deploy:read",
